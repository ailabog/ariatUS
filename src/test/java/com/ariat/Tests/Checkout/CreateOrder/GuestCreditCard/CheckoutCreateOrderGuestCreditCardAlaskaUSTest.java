...
public class CheckoutCreateOrderGuestCreditCardAlaskaUSTest extends BaseTest {
	...
	public static final String ADDRESS = "Academy Drive";
	public static final String CITY = "Anchorage";
	public static final String STATE = "Alaska";
	public static final String ZIP_CODE = "99507";
	...
	public void checkoutCreateNewOrderNotBeingLoggedMasterCardArizona() {
		logger.info("Starting Alaska checkout -> create new order without being logged credit card Master Card test...");
		...
		checkoutProcessPage.setInfoAccountSecureCheckoutUS(FIRST_NAME, LAST_NAME, ADDRESS, CITY, ZIP_CODE, MOBILE, CredentialsUtils.getProperty("email"), state.ALASKA);
		...
		logger.info("Finishing Alaska checkout -> create new order without being logged credit card Master Card test.");
	} 
	
	
	@Test(priority = 1)
	public void checkoutCreateNewOrderNotBeingLoggedVisaArizona() {
		logger.info("Starting Alaska checkout -> create new order without being logged credit card Visa test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.reviewOrderUS();
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info("Finishing Alaska checkout -> create new order without being logged credit card Visa test.");
	}
	
	@Test(priority = 2)
	public void checkoutCreateNewOrderNotBeingLoggedAmericanExpressArizona() {
		logger.info("Starting Alaska checkout -> create new order without being logged credit card American Express test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.reviewOrderUS();
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info("Finishing Alaska checkout -> create new order without being logged credit card American Express test.");
	} 

...
public class CheckoutCreateOrderGuestCreditCardTexasUSTest extends BaseTest {
	...
	public static final String ADDRESS = "Texas Crown Court";
	public static final String CITY = "Dallas";
	public static final String STATE = "Texas";
	public static final String ZIP_CODE = "77043";
	...
	@Test(priority = 0)
	public void checkoutCreateNewOrderNotBeingLoggedMasterCardTexas() {
		logger.info("Starting Texas checkout -> create new order without being logged credit card Master Card test...");
		...
		checkoutProcessPage.setInfoAccountSecureCheckoutUS(FIRST_NAME, LAST_NAME, ADDRESS, CITY, ZIP_CODE, MOBILE, CredentialsUtils.getProperty("email"), state.TEXAS);
		...
		logger.info("Finishing Texas checkout -> create new order without being logged credit card Master Card test.");
	} 
	
	
	@Test(priority = 1)
	public void checkoutCreateNewOrderNotBeingLoggedVisaTexas() {
		logger.info("Starting Texas checkout -> create new order without being logged credit card Visa test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.reviewOrderUS();
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info("Finishing Texas checkout -> create new order without being logged credit card Visa test.");
	}
	
	@Test(priority = 2)
	public void checkoutCreateNewOrderNotBeingLoggedAmericanExpressTexas() {
		logger.info("Starting Texas checkout -> create new order without being logged credit card American Express test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.reviewOrderUS();
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info("Finishing Texas checkout -> create new order without being logged credit card American Express test.");
	} 

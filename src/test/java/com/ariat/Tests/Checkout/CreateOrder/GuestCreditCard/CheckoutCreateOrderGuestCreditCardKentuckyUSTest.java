...
public class CheckoutCreateOrderGuestCreditCardKentuckyUSTest extends BaseTest {
	...
	public void checkoutCreateNewOrderNotBeingLoggedMasterCardKentucky() {
		logger.info(
				"Starting Kentucky checkout -> create new order without being logged credit card Master Card test...");
		...
		checkoutProcessPage.setInfoAccountSecureCheckoutUS(FIRST_NAME, LAST_NAME, ADDRESS, CITY, ZIP_CODE, MOBILE,
				CredentialsUtils.getProperty("email"), state.KENTUCKY);
		...
		logger.info(
				"Finishing Kentucky checkout -> create new order without being logged credit card Master Card test.");
	}
	...
	public void checkoutCreateNewOrderNotBeingLoggedAmericanExpressKentucky() {
		logger.info(
				"Starting Kentucky checkout -> create new order without being logged credit card American Express test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info(
				"Finishing Kentucky checkout -> create new order without being logged credit card American Express test.");
	}

...
public class CheckoutCreateOrderGuestCreditCardHawaiiUSTest extends BaseTest {
	...
	public void checkoutCreateNewOrderNotBeingLoggedMasterCardHawaii() {
		logger.info(
				"Starting Hawaii checkout -> create new order without being logged credit card Master Card test...");
		...
		checkoutProcessPage.setInfoAccountSecureCheckoutUS(FIRST_NAME, LAST_NAME, ADDRESS, CITY, ZIP_CODE, MOBILE,
				CredentialsUtils.getProperty("email"), state.HAWAII);
	...
	public void checkoutCreateNewOrderNotBeingLoggedAmericanExpressHawaii() {
		logger.info(
				"Starting Hawaii checkout -> create new order without being logged credit card American Express test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageUS = (HomePageUS) homePage.chooseEULocation(euCountry.USA, euCountry.USA.getCurrencyISO());
		...
		paymentMethodsCheckoutPage.placeOrderUS();
		logger.info(
				"Finishing Hawaii checkout -> create new order without being logged credit card American Express test.");
	}
